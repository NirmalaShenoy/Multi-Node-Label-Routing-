 ...
 *
 * method to send and receive MNLR and IP messages with timeout mechanism
 *
...
int _get_MACTest(struct addr_tuple *myAddr, int numTierAddr) {
	...
	for (; q < getCountOfTierAddr(); q++) {
     // populating contents for MNLR msg into ctrlPayLoadA
	// this is the actual tier addresses
		char tempAddrA[20];
	...
	while (1) {
		...
		if (!endNode) { // only does when node is an end node.
			...
			// see if we can fix mplr to mnlr 
					...
					if (packetFwdStatus == 0) {
						...
						if ((strlen(fwdTierAddr) == strlen(tierAddress))
							...
							if (portNameTemp != NULL) {
								...
								//this part of the code was introduced to match the sockets at the end devices
